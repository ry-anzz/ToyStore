...
export default function AdminMarcasPage() {
    ...
    const [deletingItem, setDeletingItem] = useState<Marca | null>(null);
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!response.ok) throw new Error("Falha ao salvar marca.");
            
            setPopup({ show: true, type: 'success', message: `Marca ${editingItem ? 'atualizada' : 'criada'} com sucesso!` });
            resetForm();
            fetchData();
        } catch (error: any) {
            setPopup({ show: true, type: 'error', message: error.message });
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            if (!response.ok) throw new Error("Falha ao excluir marca.");
            
            setPopup({ show: true, type: 'success', message: 'Marca excluída com sucesso!' });
    ...
    const openEditForm = (item: Marca) => { setEditingItem(item); setItemName(item.nome); setIsFormOpen(true); };

    return (
        <>
            {popup.show && <Popup type={popup.type} message={popup.message} onClose={() => setPopup({ ...popup, show: false })} />}
            <ConfirmationModal isOpen={!!deletingItem} onClose={() => setDeletingItem(null)} onConfirm={handleDelete} title="Confirmar Exclusão" message={`Tem certeza que deseja excluir a marca "${deletingItem?.nome}"?`} />

            <div>
                <h1 className="text-3xl font-bold mb-6">Gerenciar Marcas</h1>
                {!isFormOpen && <Button onClick={() => setIsFormOpen(true)}><PlusCircle size={20} className="mr-2"/> Adicionar Marca</Button>}

                {isFormOpen && (
                    <form onSubmit={handleSave} className="bg-white p-6 rounded-lg shadow-md my-6">
                        <h2 className="text-xl font-bold mb-4">{editingItem ? 'Editar Marca' : 'Nova Marca'}</h2>
                        <Input value={itemName} onChange={(e) => setItemName(e.target.value)} placeholder="Nome da marca" required className="w-full"/>
