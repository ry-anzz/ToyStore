...
export default function HomePage() {
  ...
  useEffect(() => {
    const fetchProdutos = async () => {
      ...
      try {
        ...
        setError("Falha ao carregar produtos. Verifique se o Spring Boot está rodando em http://localhost:8080.");
  ...
  // Lógica de filtragem baseada na query (q) da URL
  ...
  if (error) {
    return <div className="text-center py-16 bg-red-100 text-red-700 border border-red-400 rounded-lg"><h2 className="text-2xl font-bold">Erro de Conexão</h2><p>{error}</p></div>;
  }
  
  return (
    <div className="space-y-12">
      {/* ... (Seção Hero Banner e Barra Lateral de Filtros - Sem alteração) */}
